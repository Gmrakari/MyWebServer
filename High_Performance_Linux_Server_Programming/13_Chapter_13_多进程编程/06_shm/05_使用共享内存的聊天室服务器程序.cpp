 ...
 * filename:05_使用共享内存的聊天室服务器程序.cpp
 *
 */
...
/*
 *
 * 以下POSIX共享内存函数，编译的时候需要加上指定的链接选项 -lrt
	...
	while( !stop_child )
	{
		...
		for(int i = 0; i < number;i++)
		{
			...
			if((sockfd == connfd) && (events[i].events & EPOLLIN))
			{
				...
				/* 将客户数据读取到对应的读缓存中。
				 * 读缓存是共享内存的一段,它开始于idx*BUFFER_SIZE处,
				 * 长度为BUFFER_SIZE字节,因此，各个客户端连接读缓存是共享的
				 */
		...
		for(int i = 0; i < number;i++)
		{
			...
			if(sockfd == listenfd)
			{
				...
				else
				{
					...
					/* 记录新的客户连接数组users中的索引值,建立进程pid和该索引值之间的映射关系 */
			...
			else if(events[i].events & EPOLLIN)
			{
				...
				{
					/* 向除负责处理第child个客户连接的子进程之外的其它子进程发送消息，通知它们有客户数据要写 */
