...
/*
 * Date:2021-06-01 18:53
 * filename:02_processpool.h
 *
 */
...
/* 进程池类,将它定义为模板类是为了代码复用。其模板参数是出力逻辑任务的类 */
template <typename T>
class processpool {
private:
...
/* 从epollfd标识的epoll内核事件表中删除fd上的所有事件 */
...
/* 进程池构造函数
 * 参数listenfd是监听socket,它必须在创建进程池之前被创建，否则子进程无法直接引用它
...
void processpool<T>::run_child() {
	...
	/* 子进程需要监听管道文件描述符pipefd,因为父进程将通过它来通知子进程accpt新连接 */
