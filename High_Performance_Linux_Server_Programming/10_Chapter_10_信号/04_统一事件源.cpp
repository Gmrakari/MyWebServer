...
//很显然，信号处理函数需要尽可能地执行完毕，以确保该信号不被屏蔽太久

//解决方案：
//把信号的主要处理逻辑放到程序的主循环中，当信号处理函数被触发时，它只是简单地通知主循环程序接受到信号，
...
void sig_handler(int sig)
{
	...
	int msg = sig;
	send(pipefd[1],(char *)&msg,1,0);//将信号至写入管道,以通知主循环
	errno = save_errno;
}

/* setting signal handler function */
void addsig(int sig)
{
	struct sigaction sa;
