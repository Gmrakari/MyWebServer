...
private:
	...
	void add_timer(util_timer* timer,util_timer* lst_head)
	{
		...
		/*
		 * 遍历lst_head结点之后的部分链表，直到找到一个超时时间大于目标定时器的超时时间的结点
		 * 并将目标定时器插入该结点之前
		 */
		while(tmp)
		{
		...
		/*
		 * 如果遍历完lst_head结点之后的部分链表，仍未找到超时时间大于目标定时器的超时时间的结点，
		 * 则将目标定时器插入链表的尾部，并把它设置为链表的新的尾结点
		 *
		 */
		if( !tmp )
		{
 ...
 * 判断定时任务到期的一句是定时器的expire值小鱼当前系统的事件。从执行效率来看，添加定时器
 * 的时间复杂度是O(0)，删除定时器的时间复杂度是O(1)，执行定时任务的时间复杂度是O(1)
 */
