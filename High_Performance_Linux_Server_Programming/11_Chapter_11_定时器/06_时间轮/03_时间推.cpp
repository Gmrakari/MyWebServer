 ...
 * 这样，一旦心搏函数tick被调用，超时时间最小的的必然到期，我们就可以在tick函数中处理该定时器。
 ...
 * 否则就执行上虑操作，即交换空穴和它的父节点上的元素。
 * 不断执行上述过程，直到x可以被放入空穴，则插入操作完成。
 */
...
public:
	...
	/* 构造函数二，用已有数组来初始化堆 */
	time_heap(heap_timer** init_array,int size,int capacity) throw (std::unexpected) : cur_size(size),capacity(capacity)
	{
		if(capacity < size)
		{
			throw std::exception();
...
private:
	...
	void percolate_down(int hole)
	{
		...
		int child = 0;
		for(; ((hold * 2 + 1) <= (cur_size - 1)); hold = child)
		{
			child = hole * 2 + 1;
