...
public:
	...
	tw_timer* add_timer(int timeout)
	{
		 ...
		 * 并将该滴答数存储于变量ticks中。如果待插入定时器的超时值小于时间轮的槽
		 * 间隔SI,则将ticks向上折合为1,否则就将ticks向下折合为timeout/SI
		 */
		if(timeout < SI)
		{
			ticks = 1;
	...
	void tick()
	{
		...
		cur_slot = ++cur_slot % N; /* 更新时间轮的当前槽，以放映时间轮的转动 */
	}
private:
	/* 时间轮上槽的数目 */
...
/*
 * 对时间轮而言，添加一个定时器的时间复杂度是O(1),删除一个定时器的时间复杂度也是O(1),执行一个定时器的时间复杂度是O(n)
