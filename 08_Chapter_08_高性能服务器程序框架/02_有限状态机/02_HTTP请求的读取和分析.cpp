 ...
 * BAD_REQUENT 			 表示客户请求有语法错误
 * FORBINDDEN_REQUEST	 表示客户对资源没有足够的权限
 * INTERNAL_ERROR 		 表示服务器内部错误
 * CLOSED_CONNECTION	 表示客户端已经关闭连接了
 */
enum HTTP_CODE {NO_REQUEST,GET_REQUEST,BAD_REQUEST,FORBINDDEN_REQUEST,INTERNAL_ERROR,CLOSED_CONNECTION};
...
LINE_STATUS parse_line( char* buffer,int& checked_index,int& read_index )
{
	...
	for( ; checked_index < read_index;++checked_index )
	{
			...
			if( temp == '\r' )
			{
				/* 如果"\r"子字符碰巧是当前buffer中最后一个已经被读入的客户数据，那么这次分析没有刦到一个完整的行，
