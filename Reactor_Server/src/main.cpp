...
void handle_events(int epoll_fd, int listen_fd, struct epoll_event* events,
    ...
    for (int i = 0; i < events_num; i++) {
        ...
        if(fd == listen_fd) {
            ...
            //将请求任务加入到线程池中
            //加入线程池之前将mytimer和request分离（author:对request里面的timer成员进行初始化）
            //timer里面有request指针成员 requestData类里面有mytimer类成员
            request->seperateTimer();
            int rc = threadpool_add(tp, myHandler, events[i].data.ptr, 0);//myHandler是对任务的处理函数
            //events[i].data.ptr是用户传过来的数据（报文）作为任务处理函数的参数
        }
    }
}
 ...
 * 因为
 * 1、优先队列不支持随机访问
 * 2、即使支持，随机删除某节点后破坏了堆的结构，需要重新更新堆结构。所以对于被置为deleted的时间节点，会延迟到它超时
 * 或它前面的节点都被删除时，它才会被删除。
 * 一个点被置为deleted,它最迟会在TIMER_TIME_OUT时间后被删除。
 * 这样做有两个好处：
 * 1、是不需要优先遍历队列，省时。
 * 2、给超时时间一个容忍的时间，就是设定的超时时间是删除的下限（并不是一到超时时间就立即删除）
 * 如果监听的请求在超时后的下一次请求又一次出现了，就不再重新申请requestData节点了，
...
int main() {
     ...
     * 所以我们需要重新设置sigpipe信号回调操作函数 比如忽略操作等 使得我们可以防止调用它的默认操作
    ...
    //初始化线程池
    threadpool_t* threadpool = threadpool_create(THREADPOOL_THREAD_NUM, QUEUE_SIZE, 0);//创建一个4线程 65535工作列队长度的线程池
    ...
    while (true) {
        ...
        //处理外部io事件
        handle_events(epoll_fd, listen_fd, events, events_num, PATH, threadpool);//epoll_fd表示epoll事件表的套接字
        //listenfd表示监听套接字的描述符 events_num表示就绪io套接字的数组
