...
void requestData::handleRequest() {
    ...
    while (true) {
        ...
        if (read_num < 0) {
            ...
            //有请求出现但是读不到数据，可能是Request Aborted,或者来自网络的数据没有到达等原因
            perror("read_num == 0");
            if (errno == EAGAIN) {
        ...
        if (state == STATE_RECV_BODY) {
            //post请求报文的首部行里面必然会有content-length字段而get没有，所以取出这个字段
            //求出后面实体主体时候要取用的长度
            int content_length = -1;
            //在parse_Headers函数里面会把首部行的key value放在一个map里面headers
...
int requestData::parse_URI() {
    ...
    if (pos < 0)
        ...
        if (request_line.size() - pos <= 3)
            ...
            string ver = request_line.substr(pos + 1, 3); //从pos + 1开始向后截取3位
...
int requestData::parse_Headers() {
    ...
    if (h_state == h_end_LF) {
        str = str.substr(now_read_line_begin);//把首部行结束处到报文结束处作为字串
...
int requestData::analysisRequest() {
    if (method == METHOD_POST) {
        ...
        //如果发现请求报文里面设定了长连接，则把长连接信息写入回送报文里面
...
void requestData::handleError(int fd, int err_num, std::string short_msg) {
    ...
    body_buff += "HTTP/1.1" + to_string(err_num) + short_msg + "\r\n";
...
mytimer::mytimer(requestData* _request_data, int timeout) : deleted(false), request_data(_request_data) {
    //cout << "mytimer()" << endl;
    struct timeval now;//timeval中的tv_sec为1970年01月01日0点到创建struct timeval时的秒数,tv_usec为微妙数
    gettimeofday(&now, NULL);//获取系统当前时间到1970年01月01日0点时struct timeval秒数，tv_usec微秒数
    //以毫秒计
    //转化为毫秒 即设定当前定时器在距离当前timeout毫秒后超时
